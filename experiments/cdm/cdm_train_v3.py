#!/usr/bin/env python3
"""
cdm_train_v3.py — Training script for CDM V3

V3 vs V2:
  - Learnable per-slot alpha decay rates (σ(α_k) per slot)
  - Load Balancing Loss (LBL) alongside marginal entropy reg
  - Logs learned alpha values at each probe interval (diagnostic)
  - Everything else identical to V2 for clean comparison

Archon + Aura — DuoNeural — 2026-06-12
"""
...
# identical to V2 architecture params — only adds lbl_coeff
FULL_CFG = dict(
    d_model=384, n_layers=8, n_heads=8, n_kv_heads=4, d_ff=1024, K=16, max_len=512,
    entropy_reg=0.02,
    lbl_coeff=0.01,       # V3 addition: load balancing loss
    alpha_init=0.0,       # V3 addition: sigmoid(0) = 0.5 initial decay per slot
...
def log_alpha_stats(model: CDMLanguageModelV3, logfile=None):
    """Print learned alpha values per slot per layer — key V3 diagnostic."""
    stats = model.get_alpha_stats()
    log("  [V3 alpha stats] — σ(α_k) per slot (0=slow/deep, 1=fast/volatile):", logfile)
    for layer_key, slot_dict in stats.items():
        vals = list(slot_dict.values())
        mn, mx, mu = min(vals), max(vals), sum(vals)/len(vals)
        spread = [f"{v:.3f}" for v in vals]
        log(f"    {layer_key}: min={mn:.3f} max={mx:.3f} mean={mu:.3f}  [{', '.join(spread)}]", logfile)
    return stats
...
def train(cfg_dict: dict, out_dir: Path, logfile: Path = None, smoke: bool = False):
    ...
    log("=" * 60, logfile)
    log("CDM V3 — Learnable Alpha + Load Balancing Loss", logfile)
    ...
    model = CDMLanguageModelV3(arch).to(DEVICE)
    ...
    run_cfg = {**cfg_dict, "vocab_size": arch.vocab_size, "n_params": n,
               "architecture": "CDM_V3", "device": DEVICE}
    ...
    while step < STEPS:
        ...
        if step % VAL_EVERY == 0 and val_iter is not None:
            ...
            # V3 diagnostic: log learned alpha values
    ...
    # Final alpha stats
    log("\nFinal learned alpha values:", logfile)
    log_alpha_stats(model, logfile)
...
def main():
    ...
    parser.add_argument("--out", default="/workspace/cdm_v3_full")
    parser.add_argument("--log", default="/workspace/cdm_v3_training.log")
    ...
    if args.lbl_coeff is not None:
        cfg["lbl_coeff"] = args.lbl_coeff
