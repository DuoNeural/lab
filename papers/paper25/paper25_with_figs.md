...
![Figure 1: 2-Qubit RQC Architecture. The circuit shows qubit 0 (input port) receiving data via Rx gates after mid-circuit reset, while qubit 1 (memory register) propagates the accumulated parity state. The q0 reset makes the full temporal evolution a quantum channel (CPTP map), not pure unitary. The same 4-parameter ansatz θ is applied at every timestep.](figs/png/fig1_qrnn_circuit.png)
...
![Figure 2: Archon's trainability cliff. Left: convergence rate across T=3 (5/5 seeds), T=4 (2/5 seeds — unreliable training regime), T=5 (0/5 seeds). Right: mean accuracy and loss showing the sharp onset of initialization sensitivity between T=3 and T≥4.](figs/png/fig2_trainability_cliff.png)
...
![Figure 3: Aura's readout fidelity decay. Left: normalized prediction margin M(T) vs sequence length, with exponential fit τ_L=49.63. Right: generalization accuracy of fixed θ* at T=3 to 100, showing τ*(95%)=38 (ratio=0.766, inside DHP window) and τ*(99%)=28 (ratio=0.564, outside window).](figs/png/fig3_coherence_decay.png)
...
![Figure 4: Lindblad noise robustness sweep. Left: τ_L (LL fit) and τ*(95%) vs noise level — τ_L compresses with noise while τ* stays pinned near 36, driving the ratio above 1.0 in high-noise regimes. Right: DHP ratio vs noise with confirmation window [0.65, 0.79] shaded; noiseless gives 0.727 (exact classical match), low-noise gives 0.771 (inside window), medium/high/severe noise falls outside.](figs/png/fig4_lindblad_noise.png)
...
| **$\tau_L$ proxy** | $T_\text{fail} = 4$ (discrete cliff) | LL fit: $\tau_L = 49.49$ steps |
| **$\tau^*$ proxy** | $T_\text{converge} = 3$ | 95% accuracy threshold: 36–38 steps |
| **Ratio** | **0.75** | **0.727** (LL fit) / 0.766 (exp fit) |
| **Classical target** | 0.72 | 0.72 |
| **Deviation (LL)** | +4.2% | **+0.1%** |
...
![Figure 5: Dual-probe DHP synthesis. Left: all DHP confirmations across substrates. Right: both quantum probes (0.750 and 0.766/0.727) bracketing the classical 0.72 target in the DHP window [0.65, 0.79].](figs/png/fig5_dhp_synthesis.png)
